...
- [ ] The Sources panel lists ZCode as a first-class Source with enable toggle, Duration threshold, and the per-Source channel grid; default state is disabled
- [ ] The Hooks panel offers install / status / preview / uninstall for ZCode and shows the actual installation state
- [ ] ZCode never appears in any watch-source listing (it has no Watch path)
- [ ] English and Simplified Chinese locale files contain all new strings; no missing-translation warnings at runtime
- [ ] A production build of the desktop UI passes
