...
- 根据 `forked_from_id` 定位源 session，流式收集继承 `turn_id`。
...
- 识别子 session 的 `thread_settings_applied` 边界；无可靠边界时，通过首个非继承 `turn_id` 识别新轮次。
- 进入 `waiting_for_new_user` 后仍不发通知。
- 边界后首条新用户消息将 session 切换为 `live`，之后复用现有通知逻辑。
- seed priming 后同步 follower EOF，避免 attach 期间新写入内容被重复处理。

## 任务 3：补充回归测试

**文件：** `tests/watch-codex-multi-session.test.js`

- Fork 复制历史重写为当前时间时不提醒。
- Watch 附着后分批追加复制历史时不提醒。
- 没有 `thread_settings_applied` 时，首个非继承 `turn_id` 的新轮次立即恢复提醒。
- attach 与 seed priming 之间追加的内容不会被二次读取。
