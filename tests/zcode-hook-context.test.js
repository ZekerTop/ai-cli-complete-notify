...
test('non-Stop events and non-object payloads are rejected', () => {
  assert.equal(getZcodeHookNotificationContext(null, 'task'), null);
  assert.equal(getZcodeHookNotificationContext('Stop', 'task'), null);
  assert.equal(getZcodeHookNotificationContext({}, 'task'), null);
